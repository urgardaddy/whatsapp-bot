...
const cekWaktuCL = (client, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            client.sendMessage(_dir[position].id, `𝐆𝐀𝐌𝐄 𝐓𝐄𝐁𝐀𝐊 𝐂𝐀𝐊𝐋𝐎𝐍𝐓𝐎𝐍𝐆\n\n*Waktu habis*\n\n*Jawaban :* ${_dir[position].jawaban}`, MessageType.text)
            _dir.splice(position, 1)
        }
    }, 1000)
}
...
// TEBAK JENAKA
const addtebakjenaka = (chatId, jawaban, expired, _db) => {
    let obi = { id: chatId, jawaban: jawaban, expired: Date.now() + toMs(`${expired}s`) }
    _db.push(obi)
}
...
const cekWaktuTJ = (client, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            client.sendMessage(_dir[position].id, `𝐆𝐀𝐌𝐄 𝐓𝐄𝐁𝐀𝐊  𝐉𝐄𝐍𝐀𝐊𝐀\n\n*Waktu habis*\n\n*Jawaban :* ${_dir[position].jawaban}`, MessageType.text)
            _dir.splice(position, 1)
        }
    }, 1000)
}
const getTJPosi = (chatId, _db) => {
    let position = null
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === chatId) {
            position = i
